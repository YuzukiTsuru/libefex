...
int main() {
    ...
    // Test writel/readl
    uint32_t read_val;
    ret = sunxi_efex_fel_readl(&ctx, ctx.resp.data_start_address, &read_val);
    if (ret == EFEX_ERR_SUCCESS) {
        printf("Orig: 0x%08x\n", read_val);
    } else {
    ...
    ret = sunxi_efex_fel_writel(&ctx, 0x55AA55AA, ctx.resp.data_start_address);
    if (ret != EFEX_ERR_SUCCESS) {
    ...
    if (errors[0] == EFEX_ERR_SUCCESS) id[0] = id0_val;
    if (errors[1] == EFEX_ERR_SUCCESS) id[1] = id1_val;
    if (errors[2] == EFEX_ERR_SUCCESS) id[2] = id2_val;
    if (errors[3] == EFEX_ERR_SUCCESS) id[3] = id3_val;
