...
void sunxi_efex_fel_payloads_init(const enum sunxi_efex_fel_payloads_arch arch) {
...
uint32_t sunxi_efex_fel_payloads_readl(const struct sunxi_efex_ctx_t *ctx, const uint32_t addr) {
...
void sunxi_efex_fel_payloads_writel(const struct sunxi_efex_ctx_t *ctx, const uint32_t value, const uint32_t addr) {
    if (ctx->resp.mode != DEVICE_MODE_FEL) {
