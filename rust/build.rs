...
fn main() {
    ...
    build.file(src_dir.join("efex-protocol.c"));
    build.file(src_dir.join("usb_layer.c"));
    build.file(src_dir.join("efex-payloads.c"));
    build.file(src_dir.join("arch").join("riscv32_e907").join("efex-payloads.c"));
    ...
    let bindings = bindgen::Builder::default()
        ...
        .header(include_dir.join("efex-protocol.h").to_str().unwrap())
        .header(include_dir.join("usb_layer.h").to_str().unwrap())
        .header(include_dir.join("efex-payloads.h").to_str().unwrap())
